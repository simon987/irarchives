...
function statusbar(text) {
    gebi("status.cgi").innerHTML = text;
}

// Output (appends text)
function output(text) {
    gebi("output").innerHTML += text + "<br>";
}

function output_posts(text) {
    gebi("output_posts").innerHTML = text;
}

function output_comments(text) {
    gebi("output_comments").innerHTML = text;
}

function output_related(text) {
    gebi("output_related").innerHTML = text;
}

// Redirect to the page so the URL changes & we know what image is being searched
function redirect_search() {
    let url = gebi("url").value;
...
function handleSearchResponse(responseText) {
    ...
    if (resp['images'] != null) {
        ...
        for (var i = 0; i < resp.images.length; i++) {
            ...
            let thumb = '';
            const USE_IMGUR_BY_DEFAULT = true;
            if (USE_IMGUR_BY_DEFAULT || resp.images[i].thumb == null) {
                const tempi = url.lastIndexOf('.');
...
function display_comment(comment) {
    let txt = '';
    const score = comment.ups - comment.downs;
    const hexid = comment.hexid;
...
function handleGetSubredditsResponse(responseText) {
    ...
    let output = '<div class="subreddits_header">monitoring ' + subreddits.length + ' subreddits</div><br>';
...
function checkURL() {
    const query = parent.document.URL;
    if (query.indexOf('?url=') >= 0) {
        let url = query.substring(query.indexOf('?url=') + 5);
        ...
        const user = query.substring(query.indexOf('?user=') + 6);
...
function menu_subreddit_click() {
    const menu = gebi("subreddit_menu");
    if (menu.className === 'menuActive') {
        collapseMenu();
        return;
    }
...
function init() {
    ...
    if (!checkURL()) {
        ...
        const urls = [
            'http://i.imgur.com/Xz42HQa.jpg',
            'http://i.imgur.com/IFdWn.jpg',
            'http://i.imgur.com/3qrBM.jpg',
            'http://i.minus.com/ibu7TXSVaN73Nn.gif',
            'http://i.imgur.com/O1IXj.jpg',
            'http://i.imgur.com/QNj8w.jpg',
            'http://i.imgur.com/xA1wr.jpg',
            'http://i.imgur.com/54SAK.jpg',
            'http://i.imgur.com/EpMv9.jpg',
            'http://i.imgur.com/9VAfG.jpg',
            'http://i.imgur.com/OaSfh.gif',
            'http://i.imgur.com/iHjXO.jpg',
            'http://i.imgur.com/IDLu8.jpg',
            'http://i.imgur.com/ReKZC.jpg',
            'http://i.imgur.com/mhvSa.jpg',
            'http://i.imgur.com/qfzpA.jpg'
        ];
        if (gebi('url') != null) {
